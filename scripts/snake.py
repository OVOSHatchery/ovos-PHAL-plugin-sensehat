...
def draw_snake(t):
    ...
    for i in range(0, int(old_div(len(snake_pos), 2))):
        if i == 0:
            ...
            sense.set_pixel(snake_pos[2 * (i)], snake_pos[2 * (i) + 1], 0, 127,
                            255)
        else:
            # Draw all remaining segments as the darkest blue
            sense.set_pixel(snake_pos[2 * (i)], snake_pos[2 * (i) + 1], 0, 0,
                            255)
...
while True:
    ...
    for i in range(1, int(old_div(len(snake_pos), 2)) - 1):
        ...
        if not end:
            ...
            if snake_pos[0] == snake_pos[2 * i] and snake_pos[1] == snake_pos[
                2 * i + 1]:
